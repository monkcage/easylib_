...
{
  public:
    AresResolver(easy::EventLoop* loop, size_t timeout);
    ~AresResolver();
                         ...
                         const Callback& cb) override
    {
        ...
        InetAddress inet;
        {
            ...
            if (iter != globalCache().end())
            {
                ...
                if (timeout_ == 0 ||
                    cachedAddr.second.after(timeout_) > easy::Date::date())
                {
  ...
  private:
    ...
    void init();
    easy::EventLoop* loop_;
    ares_channel ctx_{nullptr};
    bool timerActive_{false};
    using ChannelList = std::map<int, std::unique_ptr<easy::Channel>>;
    ChannelList channels_;
    static std::unordered_map<std::string,
                              std::pair<struct in_addr, easy::Date>>&
    globalCache()
    {
        static std::unordered_map<std::string,
                                  std::pair<struct in_addr, easy::Date>>
